...
function isHolidayDate(dateStr) {
  const row = get(
    'SELECT * FROM badminton_holidays WHERE date = ?',
    [dateStr]
  );
  return !!row;
}
...
app.put('/api/books/:id', authMiddleware, (req, res) => {
  ...
  if (!isOwner && !isAdmin) {
    return res.status(403).json({ error: 'Only owner or admin can edit this book' });
  }

  run(
    'UPDATE books SET title = ?, author = ? WHERE id = ?',
    [title || book.title, author || book.author, id]
  );
...
app.delete('/api/books/:id', authMiddleware, (req, res) => {
  ...
  if (!isOwner && !isAdmin) {
    return res.status(403).json({ error: 'Only owner or admin can delete this book' });
  }
...
app.put('/api/machines/:id', authMiddleware, (req, res) => {
  ...
  run(
    'UPDATE machines SET name = ?, note = ?, status = ? WHERE id = ?',
    [newName, newNote, newStatus, id]
  );
...
app.post('/api/badminton/holidays', authMiddleware, requireAdmin, (req, res) => {
...
app.delete(
  ...
  (req, res) => {
    const id = Number(req.params.id);
    const item = get(
      'SELECT * FROM sports_items WHERE id = ?',
      [id]
    );
    if (!item) {
...
app.post('/api/badminton/book', authMiddleware, (req, res) => {
  ...
  if (sameDayBookings.length >= 2) {
    return res
      .status(400)
      .json({ error: 'you can only book 2 slots per day' });
  }
  ...
  const booking = get(
    'SELECT * FROM badminton_bookings WHERE id = ?',
    [info.lastInsertRowid]
  );
  res.json(booking);
});
